...
void draw_top_border(t_data *game, int x, int y, int color)
{
	...
	while(i < TILE_SIZE)
	{
		mlx_pixel_put(game->mlx_ptr, game->win_ptr, x + i, y, color);
		i++;
	}
}
...
void draw_bottom_border(t_data *game, int x, int y, int color)
{
	...
	while(i < TILE_SIZE)
	{
		mlx_pixel_put(game->mlx_ptr, game->win_ptr, x + i, y + TILE_SIZE - 1, color);
		i++;
	}
}
...
void draw_left_border(t_data *game, int x, int y, int color)
{
	...
	while(i < TILE_SIZE)
	{
		mlx_pixel_put(game->mlx_ptr, game->win_ptr, x, y + i, color);
		i++;
	}
}
...
void draw_right_border(t_data *game, int x, int y, int color)
{
	...
	while(i < TILE_SIZE)
	{
		mlx_pixel_put(game->mlx_ptr, game->win_ptr, x + TILE_SIZE - 1, y + i, color);
		i++;
	}
}
